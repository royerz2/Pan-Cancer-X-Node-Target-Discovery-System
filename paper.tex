...
\textbf{2. Co-essentiality modules.} Pairwise Jaccard co-essentiality on binarized dependency profiles (essential/non-essential per cell line), followed by Ward's hierarchical clustering, groups genes into 3--15 survival modules~\cite{Pan2018}. Jaccard is used because the relevant structure is shared essentiality membership rather than continuous Chronos score magnitude; however, this binarization discards information about dependency strength. We note that Pearson correlation-based co-essentiality~\cite{Pan2018} is the standard alternative; a systematic comparison of Jaccard-binarized vs.\ Pearson-correlation distance matrices (varying $k$ from 3--15 by silhouette score, NMI computed between the two resulting Ward clusterings) is listed as future work. Clusters are not formally enrichment-tested against pathway databases (see Limitations).
...
\textbf{4. Cancer-specific statistical dependencies.} Genes significantly more essential in the target cancer (Welch $t$-test comparing Chronos scores in cancer-type cell lines vs.\ all other lines, BH-corrected $q < 0.05$, Cohen's $d > 0.3$~\cite{Benjamini1995,Cohen1988}) form an additional survival mechanism. This comparison confounds lineage-specific and cancer-specific effects: genes essential in lung cancers may reflect shared epithelial dependencies rather than true cancer-specific vulnerabilities. Lineage-aware regression models (e.g., regressing Chronos scores on lineage covariates before cancer-type comparison) would better isolate true cancer-specific dependencies and are a priority for future versions (see Future Work).
...
Ranked triples serve as the primary therapeutic hypothesis, enforcing tri-axial diversity via the hub-gene penalty (Equation~3). Target concordance against the 43-entry gold standard (\cref{fig:benchmark}; Table~\ref{tab:benchmark}) yields:
...
To quantify each pipeline module's contribution, we re-ran the full pipeline six times, each with one component disabled, and re-evaluated benchmark concordance (Table~\ref{tab:ablation}).

\begin{table*}[htbp]
\centering
\caption{Component ablation: benchmark concordance (43 gold-standard entries) after removing individual pipeline modules. $\Delta$ denotes absolute change vs.\ full pipeline. Predictions column shows the number of cancer types yielding ranked triples.}
\label{tab:ablation}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Condition} & \textbf{AnyOvlp} & \textbf{PairOvlp} & \textbf{Exact} & \textbf{Precision} & \textbf{Preds} \\
\midrule
Full pipeline         & 46.5\% & 32.6\% & 7.0\% & 47.1\% & 17 \\
No OmniPath paths     & \phantom{0}7.0\% ($-$39.5) & \phantom{0}0.0\% ($-$32.6) & 0.0\% & 100\%  & \phantom{0}2 \\
No perturbation priors & 23.3\% ($-$23.2) & 11.6\% ($-$21.0) & 0.0\% & 29.4\% & 17 \\
No co-essentiality     & 30.2\% ($-$16.3) & \phantom{0}9.3\% ($-$23.3) & 0.0\% & 41.2\% & 17 \\
No hub penalty         & 30.2\% ($-$16.3) & 20.9\% ($-$11.7) & 2.3\% & 47.1\% & 17 \\
No statistical deps    & 48.8\% ($+$2.3)  & 32.6\% (\phantom{$+$}0.0) & 7.0\% & 47.1\% & 17 \\
\bottomrule
\end{tabular}
\end{table*}

OmniPath signaling paths are the single most important module: removing them collapses any-overlap from 46.5\% to 7.0\% and reduces predictions from 17 to 2 cancer types, because most survival mechanisms are inferred via directed network paths. Perturbation priors contribute the second-largest effect ($-$23.2 pp any-overlap, $-$21.0 pp pair-overlap); without them, feedback-aware target selection is lost and exact concordance drops to zero. Co-essentiality modules account for $-$16.3 pp any-overlap and $-$23.3 pp pair-overlap; their removal disproportionately reduces pair-level concordance because co-essential gene clusters capture pathway-level dependencies that single-gene OmniPath paths miss. The hub-gene penalty is critical for target diversity: disabling it allows STAT3 to dominate all triples, reducing any-overlap from 46.5\% to 30.2\% and pair-overlap from 32.6\% to 20.9\%. Cancer-specific statistical dependencies have minimal independent effect ($+$2.3 pp), suggesting their signal is largely captured by the other modules.

\textbf{Permutation test.} Benchmark significance was assessed via permutation test rather than a binomial test (which assumes independence across cancer types). For each of 1{,}000 iterations, within each cancer's selective-essential gene pool (median 1{,}717 genes), three genes were sampled uniformly at random; gold-standard concordance was computed pooled across 17 evaluable cancers. The observed any-overlap (44.2\%) far exceeded the null mean (0.08\%; $p < 0.001$); pair-overlap (30.2\% vs.\ 0.03\%; $p < 0.001$) and exact concordance (7.0\% vs.\ 0.0\%; $p < 0.001$) were likewise significant. Zero of 1{,}000 null iterations matched or exceeded the observed values for any metric.

\textbf{Degree-preserving network null for STAT3.} To test whether STAT3's MHS recurrence is driven by OmniPath hub degree rather than biological signal, we performed a degree-preserving edge-swap randomization test. For each of 20 permutations, OmniPath edges were shuffled ($10\times|E|$ swap attempts preserving in/out degree per node), the full pipeline was re-run on 5 test cancers (NSCLC, Melanoma, CRC, PDAC, Breast), and STAT3 frequency in MHS solutions was recorded. STAT3 appeared in 5/5 cancers under the real network but only $0.60 \pm 1.07$ cancers under the null (range 0--4; $p < 0.05$, one-sided permutation test). Under the null, CCND1 (96 appearances across $20 \times 5 = 100$ cancer--permutation pairs) and CDK4 (58) dominated, consistent with their co-essentiality module membership being independent of network topology. STAT3 appeared in only 12/100 null pairs vs.\ 5/5 observed, confirming that its recurrence reflects biological signal beyond hub degree.
...
\textbf{Computational validation and ablation.} (1)~\textbf{Lineage-aware evaluation}: evaluate within lineages (e.g., lung cancers only) to reduce confounding by lineage composition; (2)~\textbf{proliferation control}: compare predicted targets against core proliferation gene sets to distinguish cancer-specific vulnerabilities from assay artifacts; (3)~\textbf{external validation}: use PRISM/GDSC drug sensitivity data to test whether predicted target pairs/triples correspond to drug sensitivity patterns; (4)~\textbf{bootstrap stability}: bootstrap across cell lines and report stability of top-$k$ targets and triples.

\textbf{Methodological.} (1)~Subtype-specific analysis (HER2+/TNBC, EGFR-mutant/ALK+ NSCLC) to address benchmark misses; (2)~integration of CCLE expression/CNV data; (3)~data-fitted ODE models with phosphoproteomics time-courses; (4)~expanded gold standard ($>$60 entries including KRAS G12D inhibitors)~\cite{Menden2019,Holbeck2017}; (5)~genome-wide pathway enrichment of survival mechanisms (MSigDB, Reactome); (6)~true held-out cross-validation; (7)~LINCS L1000 integration replacing curated perturbation data~\cite{Subramanian2017}; (8)~lineage-aware models (regression with lineage covariates) for cancer-specific dependency testing; (9)~correlation-based co-essentiality comparison (Pearson/Spearman) as an alternative to Jaccard binarization.
