...
\begin{abstract}
Cancer drug resistance arises when tumors shift signaling through compensatory pathways. Liaki et al.~\cite{Liaki2025} showed that simultaneously targeting three independent signaling axes prevented resistance in pancreatic cancer for $>$200 days. We present ALIN (Adaptive Lethal Intersection Network), a hypothesis-generation pipeline that infers cancer-specific survival mechanisms from DepMap CRISPR data and OmniPath signaling networks, identifies minimal hitting sets (MHS) covering all inferred mechanisms, and ranks \emph{algorithmic triads}---three-gene combinations scored for predicted synergy, resistance risk, toxicity, and druggability.

Across 77 cancer types with predictions, target concordance against 43 independently curated gold-standard combinations yields 44.2\% any-overlap, 30.2\% pair-overlap, and 7.0\% exact concordance ($p < 0.001$, permutation test). Given five FDA-approved doublets, ALIN's predicted third target matches clinical evidence in 5/9 cases (56\%; $p \leq 10^{-5}$, 100{,}000 simulations). A recurrent architecture emerges---STAT3 as a pan-cancer survival hub, cell cycle regulators, and cancer-specific oncogenes---though hub-gene corrections are needed to mitigate network-degree bias. All outputs are computational hypotheses requiring experimental validation; concordance measures target overlap with historical drugs, not clinical efficacy.
\end{abstract}

\begin{keywords}
cancer | drug resistance | algorithmic triads | combination therapy | DepMap | STAT3 | pathway shifting | precision oncology
\end{keywords}
...
We present ALIN (Adaptive Lethal Intersection Network), a computational pipeline that addresses this question (\cref{fig:pipeline}). ALIN infers cancer-specific survival mechanisms from DepMap~\cite{Dempster2021} and OmniPath~\cite{Turei2026} data, identifies target combinations covering every inferred mechanism via \textbf{minimal hitting set (MHS)} optimization~\cite{VeraLicona2013,Murakami2017}, and generates ranked algorithmic triads scored for estimated synergy, resistance risk, and toxicity. Our contributions: (1)~identification of algorithmic triads covering inferred survival mechanisms across 77 cancer types as a hypothesis-generation tool; (2)~a recurrent target architecture with a hub-gene penalty addressing STAT3's algorithmic over-representation; (3)~target-concordance evaluation against 43 independently curated gold-standard combinations with four baselines and circularity ablation; (4)~multi-source pharmacological evidence integration. We claim only \textbf{algorithmic triads} (Definition~1), not mechanistic tri-axiality.

\begin{tcolorbox}[colback=gray!5, colframe=gray!60, title=\textbf{Definition 1: Algorithmic triad vs.\ mechanistic tri-axiality}, fonttitle=\small, fontlower=\small]
\textbf{Algorithmic triad.} A set of three gene targets selected by the ALIN pipeline such that, collectively, they cover all computationally inferred survival mechanisms for a given cancer type while maximizing estimated synergy and minimizing predicted toxicity. The triad is a \emph{computational hypothesis}: it is optimal with respect to the pipeline's scoring criteria, not a claim about biological pathway independence.

\textbf{Mechanistic tri-axiality} (Liaki et al.~\cite{Liaki2025}). Three functionally independent signaling axes (downstream, upstream, orthogonal) whose simultaneous pharmacological inhibition prevents adaptive pathway shifting and produces durable tumor regression. Demonstrated experimentally in PDAC; requires disease-specific validation for every other cancer type.

\emph{ALIN claims only the former.} Whether an algorithmic triad constitutes a mechanistically tri-axial combination is an experimental question that this computational study cannot answer.
\end{tcolorbox}
...
We curated 43 independently sourced multi-target ($\geq$2 gene) clinically validated combinations spanning 25 cancer types~\cite{Long2014,Planchard2016,Kopetz2019,Larkin2014,Park2021,Sequist2020,Baselga2012,Turner2015,Motzer2015,Daver2022,Ciruelos2024,Bhardwaj2019,Liaki2025}. We define a \emph{testable} entry using two output-independent criteria applied before examining concordance: (i)~the cancer type has $\geq$5 cell lines in the DepMap CRISPR screen (8 entries fail; 7 cancer types have zero representation), and (ii)~every gold-standard target has $\geq$1 inhibitor at Phase~2 or above per DGIdb~5.0 / ChEMBL~34 / FDA labels (all 34 unique targets satisfy this criterion). This yields 35 testable entries; the remaining 8 are structurally untestable due to absent DepMap cancer types. We report concordance on the full 43-entry set (primary) and the 35-entry testable subset (secondary). The primary metric is \textbf{any-overlap target concordance} ($|G \cap T| \geq 1$)~\cite{Julkunen2023}---measuring partial overlap between predicted gene targets and historical drug targets, not clinical efficacy or synergy. Secondary metrics include pair-overlap ($\geq$2 genes), superset ($G \subseteq T$), exact ($G = T$), and cancer-level precision (fraction of evaluable cancer types with $\geq$1 gold-standard target recovered). Four baselines: (1)~random-global (3 genes from $\sim$18,400; 1{,}000 permutations); (2)~random-candidate (3 genes from the per-cancer selective pool; correct null; 1{,}000 permutations; $p$-values by permutation test: for each cancer, gold-standard targets are shuffled among the candidate pool; 1{,}000 iterations yield an empirical null distribution, avoiding the independence assumption of a binomial test); (3)~global-frequency (top-3 most frequent pipeline genes); (4)~driver-gene (per-cancer top-3 from TCGA/COSMIC/OncoKB).
...
Exploratory: success/failure AUC (co-essentiality) & 0.69 (bootstrap CI [0.46--0.88]; suggestive, underpowered) \\
\bottomrule
\end{tabular}
\end{table*}
...
Post-hoc, these targets can be grouped into three functional categories---STAT3 as a high-connectivity survival hub, cell cycle regulators (CCND1, CDK4/6) as proliferation-linked effectors, and cancer-specific oncogenes (KRAS, BRAF, EGFR) as driver-pathway nodes---that parallel the Liaki et al.\ downstream/upstream/orthogonal framework. However, these are \emph{post-hoc annotations of algorithmic triads} (Definition~1), not evidence of mechanistic orthogonality. Whether this recurrent architecture reflects genuine functional independence or is partly driven by network degree biases and CRISPR assay properties requires disease-specific experimental validation (see Limitations).
...
We tested ALIN's ability to predict which third target adds therapeutic value to approved doublets---a prospective-style benchmark where the pipeline must extend a known-effective pair to a triple.
...
To contextualize this hit rate, we compared against two random baselines (100{,}000 simulations each): (i)~a \textbf{uniform random gene} drawn from the 52-gene signaling network (mean 0.26/9 matches, 2.9\%; empirical $p \leq 10^{-5}$), and (ii)~a \textbf{degree/druggability-matched random gene} drawn from the 14 druggable genes at matched network degree (mean 1.33/9, 14.8\%; empirical $p \leq 10^{-5}$). ALIN's 56\% match rate is thus $>$19$\times$ the uniform baseline and $>$3.8$\times$ the degree-matched baseline, both highly significant.
...
\textbf{MHS solutions.} The ILP solver returns a single optimal MHS: \{CDK4, CDK6, SOS1, STAT3\} (cost 4.17, coverage 58.6\%). Coverage is the lowest among the three case studies, consistent with the sparser mechanism set. No alternative solutions are found (entropy $= 0$), and no constraint filters are triggered.

\textbf{Ranked triples.} The triple scorer selects \textbf{CDK4 + CDK6 + MCL1} (combined score 1.140, the worst in the benchmark), with drugs palbociclib + palbociclib + AMG-176. This prediction has three compounding problems: (i)~CDK4 and CDK6 are pharmacologically redundant (both inhibited by palbociclib), making the triple effectively a doublet; (ii)~MCL1 inhibitors (AMG-176, S64315) caused dose-limiting cardiotoxicity in Phase~1, and clinical development is paused~\cite{Caenepeel2018}; (iii)~overlapping myelosuppression between CDK4/6 inhibitors and AML's inherent cytopenias creates a high-severity red flag. Key scores: synergy 0.81 (lowest), resistance 0.26, coverage 51.7\%, druggable 2/3 (MCL1 is Phase~1 only).

\textbf{Ablation stability.} The full-pipeline prediction is CDK6+MCL1+MET, which shifts to CDK4+CDK6+MCL1 in the final published results (reflecting post-processing refinement). Disabling the hub penalty yields CDK4+CDK6+STAT3 (STAT3 replaces MCL1). Removing co-essentiality collapses the prediction to BCL2L1+ERBB2+MET (generic RTK targets with no AML specificity). The prediction's instability across ablation conditions further confirms its low confidence.

\textbf{External validation and negatives.} The validated AML combinations target fundamentally different biology: FLT3+BCL2 (venetoclax+gilteritinib; composite CR in Phase~2)~\cite{Daver2022}, IDH1+BCL2 (ivosidenib+venetoclax; CR 71\%)~\cite{DiNardo2021}, and MEN1+BCL2 (revumenib+venetoclax; NPM1-mutant AML). These operate through lineage-specific oncogene addiction (FLT3, IDH) and apoptotic priming (BCL2)---mechanisms that a CRISPR-essentiality pipeline cannot access because (i)~FLT3/IDH are not in the curated driver gene set, and (ii)~BCL2 dependency is mediated by apoptotic priming, not proliferative essentiality. Feasibility score: $F = 0.580$ (lowest of 17; rank stays \#17), with zero clinical precedent, shared CDK4/6 drugs, and MCL1 narrow therapeutic window. \textbf{Negative:} ALIN predicts the failed MCL1 inhibitor class (Phase~1 DLT: cardiotoxicity), making this a documented false positive. AML thus exemplifies the cost of a CRISPR-only pipeline: correct identification of individual dependencies (CDK6, MCL1 are indeed AML-essential) does not guarantee a translatable combination when the actionable biology requires lineage-specific and non-cell-autonomous targets.
...
A recurrent pattern across cancer types is that algorithmic triads (Definition~1) partition into three functional categories: a high-connectivity survival hub (STAT3, 78\% of MHS predictions), proliferation-linked effectors (CCND1, CDK4/6), and cancer-specific oncogenic drivers (KRAS, BRAF, EGFR). This pattern parallels the mechanistic tri-axiality demonstrated by Liaki et al.~\cite{Liaki2025} in PDAC, but we emphasize that \emph{parallel categorization is not evidence of functional orthogonality}. STAT3 is biologically plausible as a convergent survival node---constitutively activated in $>$70\% of solid tumors~\cite{Yu2004,Frank2007}---but it is also mathematically favored as a high-degree OmniPath hub in any minimum-cost hitting set. Component ablation (Table~\ref{tab:ablation}) shows that removing OmniPath paths collapses any-overlap from 46.5\% to 7.0\%, and a degree-preserving network null (20 edge-swap permutations, 5 test cancers; see Results) confirms that STAT3's recurrence exceeds hub degree alone. Nevertheless, whether any specific algorithmic triad constitutes a mechanistically tri-axial combination---three functionally independent axes whose simultaneous inhibition prevents adaptive rewiring---is an experimental question that this computational study cannot answer.

\textbf{STAT3 as candidate orthogonal axis: algorithmic selection, hub correction, and clinical evidence.} STAT3 appears in 78\% (60/77) of MHS predictions despite being rarely mutationally activated. Its selection is biologically coherent: STAT3 functions as a convergent survival node downstream of multiple oncogenic drivers. Its high frequency reflects both genuine pan-cancer activation and algorithmic hub preference (see Limitations). However, STAT3's high connectivity in signaling networks makes it a \emph{mathematically optimal} hub for hitting-set coverage: it intersects many survival mechanisms at low marginal cost. A degree-preserving network null test (20 edge-swap permutations across 5 test cancers; see Results) shows that STAT3 appears in 5/5 observed cancers vs.\ a null mean of $0.60 \pm 1.07$ ($p < 0.05$), indicating that STAT3's recurrence significantly exceeds what network degree alone would produce. The remaining STAT3 discussion should be interpreted in light of these ablation and null results. Similarly, removing OmniPath paths entirely (retaining only co-essentiality and statistical dependency modules; see Table~\ref{tab:ablation}) collapses predictions to 2 cancers, confirming that signaling paths---not co-essentiality or statistical modules---drive STAT3 selection. To address the algorithmic confound, we (i)~removed a ``consensus path'' that previously included \emph{all} selective genes with maximum confidence, which artificially inflated STAT3's and other hub genes' mechanism frequency; and (ii)~introduced hub-handling strategies in ranked triple scoring (Equation~3). A systematic comparison of five strategies (Results; Supplementary Table~S6) shows that the original proportional penalty ($1.5\times$ excess over median) is counterproductive: it substitutes STAT3 dominance with MAP2K1 dominance (82\% max-frequency) while reducing benchmark concordance by 11.6~pp. \textbf{PageRank-normalized path coverage} achieves the best balance---matching no-penalty concordance (34.9\% any-overlap) while achieving the most equitable target distribution (Gini 0.318) and reducing STAT3 to 47.1\% of triples. With corrected survival mechanism inference and evidence-aware exemptions, STAT3 remains biologically relevant where supported by experimental data (e.g.\ PDAC via Liaki et al.~\cite{Liaki2025}), while cancer-specific oncogene targets (EGFR, MET, MAP2K1, BRAF) dominate most predictions. Hub handling does not affect MHS predictions (which remain a static combinatorial optimization) but ensures that ranked triples better reflect cancer-specific biology rather than network topology.
...
\textbf{5. Benchmark limitations and circularity.} Target concordance measures partial overlap between predicted gene targets and historical drug targets; it does not measure clinical efficacy, pharmacological synergy, or resistance prevention. ``Any-overlap'' is permissive and can reward hub genes; ``exact match'' is brittle and can miss therapeutically relevant near-misses. Many approved combinations are not correctly represented as simple gene target sets due to polypharmacology and pathway-level effects. Of 24 unmatched gold-standard entries, 8 are structurally untestable (cancer types absent from DepMap); the remaining 16 misses occur in testable entries, primarily due to modality-specific targets (KDR, PARP1, ESR1) and hematologic dependencies (FLT3, BCL2, IDH1/2). Conversely, ALIN also produces false positives: 8/77 predictions are flagged as biologically implausible, and 4/8 curated failed clinical combinations are predicted (Supplementary Table~S5; Supplementary Table~S5; see Results). The ``testable'' subset is defined using two criteria that are \emph{independent of pipeline outputs} (DepMap cell-line count $\geq 5$ and Phase~2+ druggability per DGIdb/ChEMBL; see Methods) and is applied \emph{before} examining concordance. The gold standard was curated independently from clinical sources~\cite{Long2014,Kopetz2019,Turner2015,Daver2022}; a comprehensive circularity ablation (Results; Supplementary Figure~S14) confirms no leakage: disabling \emph{all} literature-curated scoring features---KNOWN\_SYNERGIES, RESISTANCE\_MECHANISMS, perturbation bonus, combination toxicity, and evidence exemptions---produces identical predictions ($\Delta = 0.0$~pp across all metrics), while degree-matched ($p = 0.030$) and network-scramble ($p = 0.048$) nulls confirm that ALIN's concordance is not attributable to degree or druggability selection bias.
...
Inspired by Liaki et al.'s~\cite{Liaki2025} demonstration of mechanistic tri-axiality in PDAC, ALIN identifies algorithmic triads (Definition~1) covering inferred survival mechanisms across 77 cancer types. These triads are computational hypotheses, not validated regimens. A recurrent target architecture emerges---STAT3 as a high-frequency survival hub, cell cycle regulators as proliferation-linked effectors, and cancer-specific oncogenes as driver-pathway nodes---that parallels, but does not establish, mechanistic tri-axiality. A systematic comparison of five hub-handling strategies reveals that the original proportional penalty is counterproductive; PageRank-normalized path coverage achieves the best balance of benchmark concordance and target diversity (Supplementary Table~S6).

Against 43 gold-standard combinations, ALIN achieves 7.0\% exact target concordance, 30.2\% pair-overlap, and 44.2\% any-overlap ($p < 0.001$ vs.\ random-candidate null, permutation test), exceeding all baselines at pair-overlap level ($1.85\times$ vs.\ driver-gene). ALIN's third-target predictions match independently curated clinical evidence in 5/9 cases ($p \leq 10^{-5}$ vs.\ both uniform-random and degree-matched baselines). An exploratory success/failure classifier shows a suggestive trend (co-essentiality AUC~$= 0.69$, CI crossing 0.5) that is underpowered to reach significance at $n = 30$; it serves as hypothesis-generating evidence for future validation, not a clinical discriminator. These algorithmic triads are computational hypotheses for preclinical testing, not predictions of clinical efficacy. The gaps between CRISPR knockout and pharmacological inhibition~\cite{Lin2017,Goncalves2020}, between single-gene and combination screens~\cite{Norman2019}, and between algorithmic triads (Definition~1) and experimentally validated mechanistic tri-axiality demand caution in interpretation.
