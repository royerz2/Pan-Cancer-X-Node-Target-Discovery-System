...
Several high-value targets remain in the ``druggability gap'': CCND1 (no direct inhibitor; functionally covered by CDK4/6i), BCL2L1 (navitoclax limited by on-target thrombocytopenia), MCL1 (cardiotoxicity-related clinical holds on AMG-176 and S64315), and STAT3 (no approved inhibitor; Phase~2 candidates show limited single-agent activity). For these targets, protein-level data---structural druggability from AlphaFold/PDB, degradability predictions from PROTACdb, and protein--protein interaction surface analysis---could refine the scoring by distinguishing structurally tractable from genuinely undruggable proteins. Integration of mass-spectrometry proteomics (e.g., DepMap CCLE proteomics) would further allow protein-abundance-aware filtering, removing targets whose gene-level essentiality does not translate to protein-level expression in the cancer of interest. These extensions are planned for the next pipeline version (see Future Work in main text).
