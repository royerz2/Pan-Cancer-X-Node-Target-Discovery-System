...
\textbf{Protein-level druggability extension.} The current pipeline operates at the gene level, where CRISPR knockout provides the strongest functional evidence for target essentiality. However, gene-level predictions do not capture protein-level nuances critical for drug development: post-translational modification states (e.g., pSTAT3 vs.\ total STAT3), protein--protein interaction surface accessibility, structural pocket availability, and degradability by PROTACs/molecular glues. A planned extension will integrate: (1)~AlphaFold~\cite{Jumper2021} and PDB structural data for pocket-based druggability scoring; (2)~DepMap CCLE proteomics and RPPA data for protein-abundance filtering; (3)~PROTACdb and PROTAC-pedia for degradability prediction of currently ``undruggable'' targets; and (4)~phosphoproteomics time-course data to inform the ODE pathway-shifting model with empirical signaling dynamics. These protein-level features would augment---not replace---the gene-level scoring, adding a translational refinement layer that bridges CRISPR-based target discovery with clinical drug development (Supplementary Methods~S3a).
