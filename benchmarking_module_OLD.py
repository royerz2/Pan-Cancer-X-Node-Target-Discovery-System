...
GOLD_STANDARD = [
    ...
    {
        'cancer': 'Melanoma',
        'targets': frozenset({'BRAF', 'MAP2K1'}),  # dabrafenib + trametinib (MEK1/2 inhibitor)
        'evidence': 'FDA_approved',
        'description': 'BRAF + MEK1/2 (dabrafenib + trametinib) - standard of care. Trametinib inhibits both MAP2K1 and MAP2K2; counted once to avoid inflating recall.',
        'refs': ['FDA:2014', 'PMID:24295639']
    },
    # NOTE: BRAF+MAP2K2 removed — same drug regimen as BRAF+MAP2K1
    # (trametinib targets both MEK1/MEK2). Kept MAP2K1 entry; MAP2K2
    # equivalence handled by GENE_EQUIVALENTS for matching purposes.
    {
...
def run_poolmatched_baseline(triples_csv: str, n_trials: int = 100, seed: int = 42,
                              reports_dir: str = None) -> Dict:
    """
    Candidate-pool-matched baseline: for each cancer, sample random triples
    from that cancer's candidate gene pool (genes appearing in its top-5
    alternative triples and MHS targets), not the global pool.  This tests
    whether ALIN's scoring/ranking adds value beyond the gene-selection step.

    The per-cancer pool is reconstructed from the pipeline reports which list
    the best triple and up to 4 alternatives, plus the X-node MHS targets.
    Typical pool size: 5-8 genes per cancer (median 7).
    """
    import random
    import re
    import os
    ...
    # Infer reports_dir from triples_csv if not given
    if reports_dir is None:
        reports_dir = str(Path(triples_csv).parent)

    # ------------------------------------------------------------------
    # Build per-cancer candidate pools from pipeline reports
    # ------------------------------------------------------------------
    # Each report lists the best triple + up to 4 alternatives + MHS targets
    cancer_gene_pools: Dict[str, List[str]] = {}
    ...
    # Map triples-CSV cancer names → pool keys (handle _, spaces, etc.)
    csv_cancers = triples['Cancer_Type'].unique()
    cancer_pool_map: Dict[str, List[str]] = {}
    for csv_c in csv_cancers:
        ...
        if csv_c not in cancer_pool_map:
            # Unmatched: use the full output gene set as fallback
            cancer_pool_map[csv_c] = sorted(
                set(g for gs in cancer_gene_pools.values() for g in gs))

    # Count predictions per cancer for matched sampling depth
    cancer_n_preds: Dict[str, int] = {}
    for _, row in triples.iterrows():
        c = row['Cancer_Type']
    ...
    recalls = []
    for _ in range(n_trials):
        random_predictions: Dict[str, List[frozenset]] = {}
    ...
    std_r = (sum((r - mean_r) ** 2 for r in recalls) / len(recalls)) ** 0.5
    return {
        'mean_recall': mean_r,
        'std_recall': std_r,
        'n_trials': n_trials,
...
if __name__ == "__main__":
    ...
    if args.baselines:
        ...
        poolmatched_baseline = run_poolmatched_baseline(args.triples, n_trials=args.n_trials)
        ...
        metrics['poolmatched_baseline_mean'] = poolmatched_baseline['mean_recall']
        metrics['poolmatched_baseline_std'] = poolmatched_baseline['std_recall']
        ...
        print(f"  Pool-matched baseline: {poolmatched_baseline['mean_recall']*100:.1f}% ± {poolmatched_baseline['std_recall']*100:.1f}%")
...
Pool-matched (n={args.n_trials}):       {metrics.get('poolmatched_baseline_mean', 0)*100:.1f}% ± {metrics.get('poolmatched_baseline_std', 0)*100:.1f}%
